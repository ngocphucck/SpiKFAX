...
def init_params(net):
    for m in net.modules():
        if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
            ...
            # Replace init.normal_(m.weight, std=1e-3) with:
            init.kaiming_normal_(m.weight, mode='fan_in')
            if m.bias is not None:
                init.constant_(m.bias, 0)
...
def train(trainloader, net, criterion, optimizer, use_cuda=True):
    ...
    if isinstance(criterion, nn.CrossEntropyLoss):
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            # print('Inputs: ', [inputs[i].shape for i in range(len(inputs))])
            # print('Targets: ', targets)
            # exit()
            batch_size = inputs.size(0)
            total += batch_size
            inputs = rate_encode(inputs, 25)
...
def test(testloader, net, criterion, use_cuda=True):
    ...
    if isinstance(criterion, nn.CrossEntropyLoss):
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            # exit()
            # print('Outputs: ', torch.sum(outputs))
            # prin 
            loss = criterion(outputs, targets)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--spike', default=False, help='Training with spike')
